...
class DraggingProxy {
    ...
    private var mSwipeViewLayouter = SwipeViewLayouter()

    private var mLayoutCache = LayoutCache()
    ...
    fun moveView(changedView: View, positionChanges: Int) {
        ...
        for (engine in mSwipeViewLayouter.viewEngines.values) {
            engine.moveView(positionChanges.toFloat(), mSwipeViewLayouter.views[SwipeLayout.SURFACE_VIEW] as SurfaceView, changedView)
        }
    ...
    fun restoreState(state: SwipeState.DragViewState) {
        ...
        for (view in mSwipeViewLayouter.viewEngines.values) {
            view.restoreState(state, mSwipeViewLayouter.views[SwipeLayout.SURFACE_VIEW] as SurfaceView)
    ...
    fun canSwipe(swipeDirectionDetector: SwipeDirectionDetector, state: SwipeState.DragViewState): Boolean {
        ...
        val isLeftDraggable = if (mSwipeViewLayouter.DragViewEngineByPosition(SwipeLayout.LEFT_DRAG_VIEW) != null) (mSwipeViewLayouter.views[SwipeLayout.LEFT_DRAG_VIEW] as DragView).isDraggable else false
        val isRightDraggable = if (mSwipeViewLayouter.DragViewEngineByPosition(SwipeLayout.RIGHT_DRAG_VIEW) != null) (mSwipeViewLayouter.views[SwipeLayout.RIGHT_DRAG_VIEW] as DragView).isDraggable else false
