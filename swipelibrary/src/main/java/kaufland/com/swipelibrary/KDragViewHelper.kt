 ...
 */
private constructor(context: Context, private val mParentView: ViewGroup?, private val mCallback: ViewDragHelper.Callback) {
     ...
     * but [ViewDragHelper.Callback.tryCaptureView] will not be asked permission to
     * capture this view.
     *
    ...
    fun flingCapturedView(minLeft: Int, minTop: Int, maxLeft: Int, maxTop: Int) {
        ...
        mScroller.fling(capturedView!!.x.toInt(), capturedView!!.y.toInt(),
                VelocityTrackerCompat.getXVelocity(mVelocityTracker!!, activePointerId).toInt(),
                VelocityTrackerCompat.getYVelocity(mVelocityTracker!!, activePointerId).toInt(),
     ...
     * The child order is determined using [ViewDragHelper.Callback.getOrderedChildIndex].
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
    ...
    fun findTopChildUnder(x: Int, y: Int): View? {
        ...
        mParentView?.let {
            ...
            for (i in childCount - 1 downTo 0) {
                val child = it.getChildAt(mCallback.getOrderedChildIndex(i))
    ...
    private fun getEdgesTouched(x: Int, y: Int): Int {
        ...
        mParentView?.let {
            if (x < it.x + edgeSize) result = result or EDGE_LEFT
            if (y < it.y + edgeSize) result = result or EDGE_TOP
            if (x > it.x + it.width - edgeSize) result = result or EDGE_RIGHT
            if (y > it.y + it.height - edgeSize) result = result or EDGE_BOTTOM
        }
    ...
    companion object {
         ...
         */
        fun create(forParent: ViewGroup, cb: ViewDragHelper.Callback): KDragViewHelper {
         ...
         * @return a new ViewDragHelper instance
         */
        fun create(forParent: ViewGroup, sensitivity: Float, cb: ViewDragHelper.Callback): KDragViewHelper {
