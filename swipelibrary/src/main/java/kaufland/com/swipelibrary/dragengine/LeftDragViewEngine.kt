...
class LeftDragViewEngine<T : DragView>(private val mLayouter: SwipeViewLayouter) : DraggingEngine<T> {
    ...
    override fun initializePosition(orientation: SwipeViewLayouter.DragDirection) {
        ...
        dragView = mLayouter.leftDragView as T
    ...
    override fun determineSwipeHorizontalState(velocity: Float, swipeDirectionDetector: SwipeDirectionDetector, swipeState: SwipeState, swipeListener: SwipeLayout.SwipeListener?, releasedChild: View): SwipeResult? {
        if (dragView == releasedChild && swipeDirectionDetector.swipeDirection == SwipeDirectionDetector.SWIPE_DIRECTION_LEFT && Math.abs(swipeDirectionDetector.difX) > dragDistance / 2) {
