...
class SurfaceViewEngine(private val mLayouter: SwipeViewLayouter) : DraggingEngine<SurfaceView> {
    ...
    override fun moveView(offset: Float, view: SurfaceView, changedView: View) {
        ...
        if (mLeftDragViewEngine != null && changedView == mLeftDragViewEngine!!.dragView) {
            dragView!!.x = mLeftDragViewEngine!!.dragView?.x?.plus(mLeftDragViewEngine!!.dragView!!.width) ?: 0f
        }

        if (mRightDragViewEngine != null && changedView == mRightDragViewEngine!!.dragView) {
            dragView!!.x = mRightDragViewEngine!!.dragView?.x?.minus(dragView!!.width) ?: 0f
        }
    }
    ...
    override fun determineSwipeHorizontalState(velocity: Float, swipeDirectionDetector: SwipeDirectionDetector, swipeState: SwipeState, mSwipeListener: SwipeLayout.SwipeListener?, releasedChild: View): SwipeResult? {
        ...
        if (swipeState.state == SwipeState.DragViewState.CLOSED) {
            if (swipeDirection == SwipeDirectionDetector.SWIPE_DIRECTION_RIGHT) {
                ...
                swipeState.state = SwipeState.DragViewState.LEFT_OPEN
                return if (velocity > LEFT_FULL_AUTO_OPEN_TRESHOLD || Math.abs(swipeDirectionDetector.difX) > mLeftDragViewEngine!!.dragDistance / 2) {
                    SwipeResult(mLeftDragViewEngine!!.dragDistance, Runnable { mSwipeListener?.onSwipeOpened(LEFT_OPEN, true) })
                } else {
                    SwipeResult(mLeftDragViewEngine!!.intermmediateDistance, Runnable { mSwipeListener?.onSwipeOpened(LEFT_OPEN, mLeftDragViewEngine!!.intermmediateDistance == mLeftDragViewEngine!!.dragDistance) })
                }

            } else if (swipeDirection == SwipeDirectionDetector.SWIPE_DIRECTION_LEFT) {
                ...
                return if (velocity < RIGHT_FULL_AUTO_OPEN_TRESHOLD || Math.abs(swipeDirectionDetector.difX) > mRightDragViewEngine!!.dragDistance / 2) {
                    SwipeResult(-mRightDragViewEngine!!.dragDistance, Runnable { mSwipeListener?.onSwipeOpened(RIGHT_OPEN, true) })
                } else {
                    SwipeResult(-mRightDragViewEngine!!.intermmediateDistance, Runnable { mSwipeListener?.onSwipeOpened(RIGHT_OPEN, mRightDragViewEngine!!.intermmediateDistance == mRightDragViewEngine!!.dragDistance) })
                }
            }
        } else if (swipeState.state == SwipeState.DragViewState.LEFT_OPEN) {
            if (swipeDirection == SWIPE_DIRECTION_RIGHT) {
                swipeState.state = LEFT_OPEN

                return SwipeResult(mLeftDragViewEngine!!.intermmediateDistance, Runnable { mSwipeListener?.onBounce(LEFT_OPEN) })

            } else if (swipeDirection == SWIPE_DIRECTION_LEFT) {

                swipeState.state = CLOSED
                return SwipeResult(0, Runnable { mSwipeListener?.onSwipeClosed(CLOSED) })
            }
    ...
    override fun initializePosition(orientation: SwipeViewLayouter.DragDirection) {
        ...
        mLeftDragViewEngine = mLayouter.DragViewEngineByPosition(SwipeLayout.LEFT_DRAG_VIEW)
        mRightDragViewEngine = mLayouter.DragViewEngineByPosition(SwipeLayout.RIGHT_DRAG_VIEW)
    }
