...
class RightDragViewEngine<T : DragView>(private val mLayouter: SwipeViewLayouter) : DraggingEngine<T> {

    override var dragView: T? = null
        private set
    ...
    override fun initializePosition(orientation: SwipeViewLayouter.DragDirection) {
        ...
        dragView = mLayouter.views[SwipeLayout.RIGHT_DRAG_VIEW] as T?
        mSurfaceView = mLayouter.surfaceView
        dragDistance = dragView!!.width
        mInitialXPos = (mSurfaceView!!.x + mSurfaceView!!.width).toInt()
        intermmediateDistance = if (dragView!!.settlePointResourceId != -1) dragView!!.findViewById<View>(dragView!!.settlePointResourceId).right else dragView!!.width
    ...
    override fun determineSwipeHorizontalState(velocity: Float, swipeDirectionDetector: SwipeDirectionDetector, swipeState: SwipeState, swipeListener: SwipeLayout.SwipeListener?, releasedChild: View): SwipeResult? {
        if (releasedChild == dragView && swipeDirectionDetector.swipeDirection == SwipeDirectionDetector.SWIPE_DIRECTION_RIGHT && Math.abs(swipeDirectionDetector.difX) > dragDistance / 2) {
